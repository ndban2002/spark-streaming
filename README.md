...
src/streaming/
Chứa logic xử lý stream: kết nối Kafka, Spark Streaming, xử lý foreachBatch,...

Có thể chia nhỏ thành nhiều file tùy độ phức tạp.

src/models/
Khai báo schema cho các bảng dim, fact (dùng kiểu Spark DataFrame Schema, hoặc pydantic nếu cần).

Dễ quản lý mapping schema và cập nhật.

src/resources/
Chứa dữ liệu cần thiết.

src/utils/
Các hàm tiện ích: logger, xử lý config, helper functions.

jobs/
Các script chạy 1 lần, ví dụ tạo bảng, kiểm tra kết nối, hoặc chạy ETL offline.

config/
Chứa các file cấu hình cho Kafka, Spark, PostgreSQL (nên dùng .env hoặc yaml/json).